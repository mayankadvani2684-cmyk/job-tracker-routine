...
def tier(job):
    ...
    if TIER1_TITLE_RE.search(title):
        if not TIER1_EXCL_RE.search(industry):
            return 1
...
def run_actor(search_url):
    payload = {"urls": [search_url], "count": COUNT, "scrapeCompany": False}
    r = requests.post(
    ...
    for _ in range(60):
        status = requests.get(
            f"{BASE}/actor-runs/{run_id}?token={APIFY_TOKEN}", timeout=30
        ).json()["data"]["status"]
        if status == "SUCCEEDED":
            break
...
def fill_sheet(ws, jobs):
    ...
    for ri, job in enumerate(jobs, 2):
        ...
        raw_date = (job.get("postedDate") or job.get("publishedAt") or
                    job.get("postedAt") or "")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        futures = {pool.submit(run_actor, url): url for url in SEARCH_URLS}
    ...
    for job in all_jobs:
        ...
        raw_url        = (job.get("link") or job.get("jobUrl") or
...
if __name__ == "__main__":
    data, jobs = main()
    print(json.dumps({"status": "ok", "rows": len(jobs), "file": FILENAME}))
